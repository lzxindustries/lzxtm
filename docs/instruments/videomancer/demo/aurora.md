...
*Aurora painting luminous Lissajous trails in rainbow mode â€” overlapping orbits create prismatic interference patterns on a persistent framebuffer.*
<img src={aurora_animation} alt="Aurora animated output"/>
*Aurora output evolving over multiple frames â€” synthesis programs generate imagery without requiring a video input source.*

---

## Overview

In the early 1990s, the Amiga demoscene invented an effect called "shadebobs" â€” small sprites drawn additively into a framebuffer without ever clearing the screen. As each bob traced its path, it left behind a trail of accumulated brightness, painting luminous interference patterns in a kind of long-exposure drawing with light. Aurora recreates this technique in real-time FPGA hardware.

The program maintains a persistent 40Ã—32 pixel framebuffer stored in BRAM (stride 64, fitting in 4 BRAM tiles at 2,048 bytes). Two or four "bobs" â€” single-pixel stamps â€” move in Lissajous orbits across this canvas. Each frame, the bobs are additively stamped into the framebuffer (brightness values are added, saturating at maximum). A global fade gradually dims the entire canvas, creating a dynamic equilibrium where new strokes glow brightly while old ones decay to darkness. The framebuffer is read out during active video, upscaled to full resolution via nearest-neighbor (each framebuffer pixel covers a 32Ã—32 block of output pixels), and colorized through one of four palette modes before mixing with the input video.

The name *Aurora* refers to the aurora borealis â€” curtains of light that shimmer and shift across the polar sky. With the heat map or rainbow colorizer and a trefoil Lissajous pattern, the resemblance is unmistakable: luminous arcs of green and pink light glowing against darkness.
...
The visual result is ethereal: overlapping dots of light create webs, spirals, and curtain-like formations. The longer the effect runs, the richer and more complex the pattern becomes. The characteristic "smeared light" quality â€” where the bob builds up brightness at intersection points â€” is unlike any other visual effect.
...
â”‚  Phase Increment (independent process, triggered at vsync)       â”‚
â”‚     â”œâ”€ For each bob: phase_x += speed Ã— preset.fx (or fixed)    â”‚
â”‚     â”‚                 phase_y += speed Ã— preset.fy (or fixed)    â”‚
â”‚     â””â”€ 16-bit phase accumulators, wrapping                       â”‚
â”‚           â—„â”€â”€ Speed (pot 1), Pattern (pot 2)                     â”‚
â”‚                                                                  â”‚
â”‚  Frame Update FSM (during vertical blanking)                     â”‚
â”‚                                                                  â”‚
â”‚  1. Bob Position Compute (BOB_COMPUTE â†’ BOB_MULT â†’ BOB_CLAMP)   â”‚
â”‚     â”œâ”€ For each active bob:                                      â”‚
â”‚     â”‚   sine_x = quarter_wave_LUT(phase_x[bob](15:8))           â”‚
â”‚     â”‚   sine_y = quarter_wave_LUT(phase_y[bob](15:8))           â”‚
â”‚     â”‚   pos = FB_center + shift_right(sine(9:4) Ã— amp(9:4), 2)  â”‚
â”‚     â””â”€ Clamp to 1..Wâˆ’2 / 1..Hâˆ’2                                â”‚
â”‚           â—„â”€â”€ Orbit Size (pot 6), Bob Count (toggle 8)           â”‚
â”‚                                                                  â”‚
â”‚  2. Global Fade (FADE_RD / FADE_WR, 2-cycle read-modify-write)  â”‚
â”‚     â”œâ”€ For each pixel in 40Ã—32 FB (2048 addresses):              â”‚
â”‚     â”‚   fb[addr] = max(0, fb[addr] âˆ’ fade_subtract)             â”‚
â”‚     â””â”€ fade_subtract = fade_rate(9:6) â†’ 0..15 per frame         â”‚
â”‚           â—„â”€â”€ Fade Rate (pot 3)                                  â”‚
â”‚                                                                  â”‚
â”‚  3. Bob Stamp (STAMP_RD / STAMP_WR, 1Ã—1 additive)               â”‚
â”‚     â”œâ”€ For each bob:                                             â”‚
â”‚     â”‚   brightness = intensity(9:2) â†’ 8-bit                     â”‚
â”‚     â”‚   fb[addr] = min(255, fb[addr] + brightness)               â”‚
â”‚     â””â”€ Saturating 9-bit addition                                 â”‚
â”‚           â—„â”€â”€ Intensity (pot 4), Bob Count (toggle 8)            â”‚
â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜

â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
â”‚  Readout Pipeline (during active video)                          â”‚
â”‚                                                                  â”‚
â”‚  Stage 1: Address generation                                     â”‚
â”‚     â””â”€ fb_x = hcount(11:5), fb_y = vcount(10:5)  (Ã·32)         â”‚
â”‚        clamped to FB_Wâˆ’1 and FB_Hâˆ’1                              â”‚
â”‚        addr = fb_y Ã— 64 + fb_x  (shift_left + OR)               â”‚
â”‚                                                                  â”‚
â”‚  Stage 2: BRAM read (1 clk latency)                              â”‚
â”‚     â””â”€ 8-bit luminance value                                     â”‚
â”‚                                                                  â”‚
â”‚  Stage 3a: Scale + pre-register                                  â”‚
â”‚     â””â”€ luma_10 = fb_byte & "00"  (8-bit â†’ 10-bit, Ã—4)           â”‚
â”‚                                                                  â”‚
â”‚  Stage 3b: Colorize (from registered fb_byte and luma)           â”‚
â”‚     â”œâ”€ Mono:    Y = luma, UV = pre-computed hue tint             â”‚
â”‚     â”œâ”€ Heat:    Y = luma, UV = 4-band map via fb_byte(7:6)       â”‚
â”‚     â”œâ”€ Rainbow: Y = luma, UV = 4-quadrant hue via                â”‚
â”‚     â”‚           fb_byte(7:6) + hue(9:8) â†’ 2-bit index            â”‚
â”‚     â””â”€ Duo:     Y = luma, UV = 2-color split on fb_byte(7)      â”‚
â”‚           â—„â”€â”€ Color Mode (toggle 7), Hue (pot 5)                 â”‚
â”‚                                                                  â”‚
â”‚  Stage 4: Brightness scaling (6Ã—6 multiply)                      â”‚
â”‚     â””â”€ Y = (color_Y(9:4) Ã— orbit_size(9:4)) >> 2                â”‚
â”‚           â—„â”€â”€ Orbit Size (pot 6)                                 â”‚
â”‚                                                                  â”‚
â”‚  Stage 5: Output register                                        â”‚
â”‚                                                                  â”‚
â”‚  Stages 6â€“9: Interpolator (4 clk, per Y/U/V)                    â”‚
â”‚     â””â”€ Mix = lerp(input_delayed, processed, mix_amount)          â”‚
â”‚           â—„â”€â”€ Mix (fader 12)                                     â”‚
â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜

 Output = bypass ? input_delayed : mix_result
           â—„â”€â”€ Bypass (toggle 11)
```

Aurora is fundamentally different from Videomancer's signal-processing programs. Instead of transforming the input video frame-by-frame, it maintains an internal canvas (framebuffer) with persistent state across frames. The input video is only used in two ways: (1) Video Seed mode paints input luminance into the framebuffer, and (2) the Mix fader crossfades between the generated image and the delayed input. The frame update FSM (bob position compute â†’ fade â†’ stamp) runs during vertical blanking using 2-cycle read-modify-write through BRAM ports; the readout pipeline runs during active video, sharing the same BRAM read port via an address mux. Phase increments run in a separate process triggered at vsync, decoupled from the FSM to avoid adding carry chains to the critical path. The Orbit Size pot serves double duty: it controls the Lissajous amplitude during bob position computation and also scales the readout luminance in the brightness scaling stage.
...
*Videomancer's front panel with Aurora active. Knobs 1â€“6 (top two rows of left cluster), Toggle switches 7â€“11 (bottom row of left cluster), Fader 12 (right side).*
...
Controls the orbital velocity of the bobs. At 0%, the bobs are frozen in place â€” they stamp the same position every frame, burning a bright static dot into the framebuffer. As you increase speed, the bobs trace their Lissajous paths faster. Very high speeds make the bobs race around their orbits so quickly that the trails blur together into continuous luminous bands. The speed pot scales phase increment per frame, so the relationship between Speed and visual velocity depends on the Pattern preset â€” complex presets with high frequency ratios produce faster apparent motion for the same Speed setting.
...
Controls the brightness of each 1Ã—1 bob stamp â€” how much luminance is added per pixel per frame. The 10-bit register is scaled to 8-bit via `intensity(9:2)`, giving a brightness range of 0â€“255 per stamp. At low intensity, bobs produce faint traces that build up slowly through repeated passes. At high intensity, each stamp burns a bright mark into the framebuffer, creating vivid trails that saturate quickly.
...
- **Heat mode**: Hue has no direct effect â€” the 4-band thermal palette is fixed.
...
- **Duo mode**: Hue has no direct effect â€” the two-color pair is fixed (complementary cyan/warm tones split on `fb_byte(7)`).
...
|--------|-----|-----|
| **7 â€” Color** | Mono | Heat |
| **8 â€” Bobs** | 2 Bobs | 4 Bobs |
| **9 â€” Clear** | Off | On |
| **10 â€” Video Seed** | Off | On |
| **11 â€” Bypass** | Off | On |

The five toggle switches control four independent binary options plus a combined 2-bit color mode selector. Toggle 7 is a 4-position selector (using 2 bits from `registers_in(6)(1:0)`) that dramatically changes the visual character. Toggles 8â€“11 are independent binary controls mapped to individual bits of register 6.
...
Wet/dry crossfade between the input video (delayed to match pipeline latency) and the Aurora output. At 0% (fully down), the output is pure input video â€” Aurora is inaudible. At 100% (fully up), the output is pure Aurora. Intermediate positions blend the two, allowing the aurora pattern to be superimposed over live video at any opacity. This is the primary creative control for compositing the generative pattern with external footage.

---

## Guided Exercises

These exercises progress from basic trail painting to complex multi-bob compositions with colorization. Because Aurora is a generative program with persistent state, each exercise unfolds over time â€” allow 30â€“60 seconds after each parameter change to observe the full effect.

### Exercise 1: Classic Shadebob Trails

<img src={aurora_exercise1_result} alt="Classic Shadebob Trails result"/>
*Classic Shadebob Trails â€” simulated result across source images.*
...
4. **Minimal fade**: Set Fade Rate to ~6%. With almost no fade, trails accumulate permanently â€” the canvas fills with a luminous long-exposure record of every orbit pass.
5. **Maximum intensity**: Set Intensity to 100%. Each 1Ã—1 stamp burns a bright mark into the framebuffer, and intersection points where both bobs cross saturate to full brightness.
...
*Rainbow Aurora â€” simulated result across source images.*
...
2. **Enable Rainbow**: Switch Color (Toggle 7) to Rainbow. The trails immediately display colored bands â€” the 4-quadrant colorizer maps luminance regions to distinct hues.
...
6. **Rotate the quadrants**: Sweep Hue (Knob 5). The 4-quadrant color assignment rotates â€” the same luminance bands map to different color regions.
...
*Video Seed Composition â€” simulated result across source images.*
...
- **Aurora is a painting program**: Unlike most Videomancer programs that transform each frame independently, Aurora accumulates over time. Be patient â€” allow 30â€“60 seconds after each control change to see the full effect evolve.
...
| **BRAM** | Block RAM; dedicated memory blocks within the FPGA used for the persistent 40Ã—32 framebuffer that stores accumulated bob trail data. |
