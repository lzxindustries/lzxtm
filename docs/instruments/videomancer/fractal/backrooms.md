...
*Backrooms output evolving over multiple frames â€” synthesis programs generate imagery without requiring a video input source.*

---

## Overview

Every generation discovers its own image of infinitely repeating, inescapable architecture. For the internet era that image is the Backrooms â€” an endlessly recursing set of empty office corridors, fluorescent-lit and yellow-carpeted, stretching in every direction with no exit and no center. Backrooms takes that concept and builds it in hardware: a real-time procedural maze generator that creates infinite scrolling labyrinths from pure computation, with no memory storage whatsoever.
...
At small cell sizes, the maze reads as an intricate grid texture overlaid on the source. At large cell sizes, it becomes an architecturally scaled labyrinth where entire objects in the source video become visible through corridor openings. The Seed parameter completely changes the maze topology â€” each of the 256 seeds produces a deterministically different layout, so the same source video can be viewed through entirely different spatial configurations.
...
â”‚
â”œâ”€â”€ Pixel Position â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
â”‚   â”‚
â”‚   â”œâ”€ 1. Pixel/Line Counters    (h_count, v_count from timing)
â”‚   â”œâ”€ 2. Scroll Offset          (accumulate or direct from pots)
â”‚   â”œâ”€ 3. Scrolled Coordinates   (pixel + scroll_offset)
â”‚   â””â”€ 4. Cell Decomposition     (cell_xy = pixel >> shift,
â”‚                                  frac_xy = pixel AND mask)
â”‚
â”œâ”€â”€ Maze Computation â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
â”‚   â”‚
â”‚   â”œâ”€ 5. Hash Level 0           (seed Ã— cell_x Ã— cell_y â†’ coarse grid)
â”‚   â”œâ”€ 6. Hash Level 1           (refine with half-cell coords)
â”‚   â”œâ”€ 7. Hash Level 2           (organic diagonal mixing)
â”‚   â”œâ”€ 8. Hash Level 3           (fine detail + final wall bits)
â”‚   â””â”€ 9. Wall Decision          (frac_xy vs threshold â†’ is_wall)
â”‚
â”œâ”€â”€ Color Output â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
â”‚   â”‚
â”‚   â”œâ”€ 10. Invert                (swap wall/corridor)
â”‚   â”œâ”€ 11. Wall Color Gen        (solid or video-modulated luma)
â”‚   â””â”€ 12. Corridor Passthrough  (input video for non-wall pixels)
â”‚
â”œâ”€â”€ Mix â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
â”‚   â””â”€ 13. Interpolator Ã— 3      (dry/wet crossfade Y, U, V)
â”‚
...
*Videomancer's front panel with Backrooms active. Knobs 1â€“6 (top two rows of left cluster), Toggle switches 7â€“11 (bottom row of left cluster), Fader 12 (right side).*
...
Controls wall thickness as a proportion of cell size. At low values, walls are thin hairlines with wide corridors â€” the source video dominates and the maze reads as a delicate overlay grid. At high values, the walls thicken and corridors narrow to slits, inverting the visual balance so the maze structure dominates with only glimpses of video visible through tight passages. Combined with Invert, this control defines whether the program feels like video-with-grid or grid-with-video.
...
Sets the horizontal scroll position or speed depending on the Animate toggle state. In manual mode, the knob directly positions the maze horizontally â€” useful for precisely framing a static composition. In animate mode, the knob controls scroll velocity: center (180Â°) is stationary, left of center scrolls leftward, right of center scrolls rightward. Higher offsets from center produce faster scrolling. The scroll wraps seamlessly because the hash function operates on the scrolled coordinates modulo the cell grid.
...
Controls the brightness of wall surfaces when in Color mode (Wall Luma toggle set to Color). At 0%, walls are black â€” the maze reads as a dark grid over bright video. At 100%, walls are maximum brightness â€” the maze reads as a bright grid. The wall color includes a subtle warm tint (slightly shifted U and V) that prevents walls from looking purely neutral, giving the maze surfaces a faintly architectural quality.
...
Selects one of 256 maze topology seeds. Each seed value produces a completely different maze layout â€” different wall placements, different corridor configurations, different connectivity. Because the hash function has good avalanche properties, even adjacent seed values produce visually unrelated mazes. This parameter is the creative "dice roll": when you find a source image you like, scrolling through seeds lets you audition completely different maze structures around the same content.
...
|--------|-----|-----|
| **7 â€” Animate** | Off | On |
| **8 â€” Maze Style** | Grid | Organic |
| **9 â€” Invert** | Off | On |
| **10 â€” Wall Luma** | Color | Video |
| **11 â€” Bypass** | Off | On |
...
*Architectural Grid â€” simulated result across source images.*
...
3. Center Scroll X and Scroll Y at 180Â° for a static maze.
...
8. Leave Invert Off â€” corridors show the source video.
...
*Infinite Scrolling Labyrinth â€” simulated result across source images.*
...
4. Set Scroll X to about 220Â° (moderate rightward drift).
5. Set Scroll Y to about 240Â° (moderate downward drift).
...
9. Set Mix to 100%.
10. Observe the infinite scroll â€” new maze structure flows continuously into frame.

**Key concepts**: Organic mode adds diagonal hash mixing and wall texture fragments, creating passages that feel eroded and natural rather than architectural. The continuous scroll demonstrates that the maze extends infinitely â€” the hash function generates consistent topology at every coordinate, with no visible tiling or repetition within the practical scroll range.
...
*Video-Modulated Maze Inversion â€” simulated result across source images.*
...
3. Set Scroll X and Scroll Y to 180Â° (static).
4. Set Animate to Off.
5. Set Maze Style to Grid.
6. Switch Wall Luma to Video â€” walls now show monochromatic source luma.
7. Set Wall Color to 80% (controls intensity modulation in video mode).
8. Enable Invert â€” now corridors show the wall color and walls show video.
...
**Key concepts**: With Wall Luma set to Video, wall pixels inherit the input's brightness while discarding chroma. When combined with Invert, this creates a figure-ground reversal where both regions show the same image but with different color treatment â€” full color in one, monochrome in the other. The maze becomes a per-pixel color/mono masking system driven by procedural geometry.
...
- **Organic mode pairs with scrolling**: The rough, textured corridors of Organic mode look most effective when the maze is continuously scrolling â€” the irregular wall surfaces create a dynamic, living quality.
- **Invert changes everything**: The visual character switches dramatically between normal and inverted modes. Try both for every composition â€” sometimes the "negative space" version is more compelling.
...
- **Video walls for texture**: Switching Wall Luma to Video mode makes walls show a monochromatic ghost of the source â€” useful for maintaining image readability across both wall and corridor regions.
...
- **Mix for subtlety**: At 30-50% mix, the maze becomes a semi-transparent grid overlay that adds texture without obscuring the source â€” effective for broadcast-style frame treatments.
