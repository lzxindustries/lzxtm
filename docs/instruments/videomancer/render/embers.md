...
â”‚   â”œâ”€ emit_rate   = registers_in(0)     [10-bit]
â”‚   â”œâ”€ gravity     = registers_in(1)     [10-bit]
â”‚   â”œâ”€ velocity    = registers_in(2)     [10-bit]
â”‚   â”œâ”€ lifetime    = registers_in(3)     [10-bit]
â”‚   â”œâ”€ size        = registers_in(4)     â†’ particle_size [1â€“8]
â”‚   â”œâ”€ spread      = registers_in(5)     [10-bit]
â”‚   â”œâ”€ brightness  = registers_in(7)     [10-bit]
â”‚   â””â”€ toggles from registers_in(6):
â”‚       â”œâ”€ bit 0: emit_mode (0=fountain, 1=video)
â”‚       â”œâ”€ bit 1: grav_dir  (0=down, 1=up)
â”‚       â”œâ”€ bit 2: drag_en   (0=off, 1=on)
â”‚       â”œâ”€ bit 3: color_mode(0=hot, 1=rainbow)
â”‚       â””â”€ bit 4: bypass
â”‚
â”œâ”€â”€ LFSR16 Instance â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
â”‚   â””â”€ lfsr16(seed=0xCAFE) â†’ 16-bit pseudo-random stream
â”‚
â”œâ”€â”€ Position Counters â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
â”‚   â”œâ”€ h_count: 12-bit horizontal pixel counter
â”‚   â””â”€ v_count: 12-bit vertical line counter
â”‚
â”œâ”€â”€ Physics Update (per vblank, 8 sequential clocks) â”€â”€â”€â”€â”€â”€â”€â”€â”€
â”‚   For each particle i = 0..7:
â”‚   â”œâ”€ Gravity: grav = gravity[9:6] signed; negate if grav_dir
â”‚   â”œâ”€ Velocity: vy += grav
â”‚   â”œâ”€ Drag (if enabled): vx -= vx>>>3; vy -= vy>>>3
â”‚   â”œâ”€ Clamp vx, vy to [-128..+127]
â”‚   â”œâ”€ Position: px += vx; py += vy  (saturating 12-bit)
â”‚   â””â”€ Lifetime: life -= 4  (clamp to 0)
â”‚
â”œâ”€â”€ Emission (concurrent with scan) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
â”‚   If emit_counter < emit_rate AND life[emit_idx] == 0:
â”‚   â”œâ”€ Fountain: px=960, py=540 (screen center)
â”‚   â”œâ”€ Video: px=h_count, py=v_count (if input Y > 512)
â”‚   â”œâ”€ vx = lfsr[7:0] Ã— spread[9:6] >> 3  (signed, spread-scaled)
â”‚   â”œâ”€ vy = -velocity[9:3]  (upward initial impulse)
â”‚   â””â”€ life = lifetime parameter
â”‚
â”œâ”€â”€ Rendering Pipeline (per pixel, combinatorial) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
â”‚   For each particle i with life > 0:
â”‚   â”œâ”€ dx = h_count âˆ’ px(i);  dy = v_count âˆ’ py(i)
â”‚   â”œâ”€ abs_dx, abs_dy
â”‚   â”œâ”€ Chebyshev: radius = max(abs_dx, abs_dy)
â”‚   â”œâ”€ if radius â‰¤ particle_size:
â”‚   â”‚   â”œâ”€ bright = (size âˆ’ radius) << shift  (distance falloff)
â”‚   â”‚   â””â”€ bright = bright Ã— life >> 10  (lifetime scale)
â”‚   â””â”€ best_bright = max over all particles
â”‚
â”œâ”€â”€ Color Mapping â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
â”‚   â”œâ”€ pixel_bright = best_bright Ã— brightness >> 10
â”‚   â”œâ”€ Hot mode:
â”‚   â”‚   â”œâ”€ bright > 768: white  (Y=bright, U=512, V=512)
â”‚   â”‚   â”œâ”€ bright > 384: orange (Y=bright, U=512âˆ’bright>>3, V=512+bright>>2)
â”‚   â”‚   â””â”€ else:         red    (Y=bright, U=512âˆ’bright>>2, V=512+bright>>1)
â”‚   â””â”€ Rainbow mode:
â”‚       â””â”€ Y=bright, U=512+bright>>2, V=512âˆ’bright>>2
â”‚
â”œâ”€â”€ Additive Composite â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
â”‚   comp_y = lerp10(input_y, color_y, pixel_bright)
â”‚   comp_u = lerp10(input_u, color_u, pixel_bright)
â”‚   comp_v = lerp10(input_v, color_v, pixel_bright)
â”‚
â”œâ”€â”€ Sync Pipeline (8-stage delay) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
â”‚   â””â”€ hsync_n, vsync_n, avid, field_n delayed 8 clocks
â”‚
â””â”€â”€ Bypass Mux â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    â””â”€ bypass ? input : composite
```

The physics engine and rendering pipeline occupy two distinct temporal domains. Physics updates run during the vertical blanking interval â€” a narrow window of approximately 45 lines at HD rates â€” processing one particle per clock in a simple sequential loop. This avoids any contention with the rendering pipeline, which runs during the active video region and must evaluate all eight particles in parallel for every pixel. The rendering stage is fully combinatorial within each clock: eight distance comparisons, eight brightness computations, and a maximum-brightness selector all resolve in a single pipeline stage. This dual-domain architecture allows Embers to maintain a consistent 8-clock total latency regardless of particle count, with zero BRAM and minimal DSP usage.
...
*Videomancer's front panel with Embers active. Knobs 1â€“6 (top two rows of left cluster), Toggle switches 7â€“11 (bottom row of left cluster), Fader 12 (right side).*
...
Sets the magnitude of gravitational acceleration applied to each particle's vertical velocity on every frame. At zero, particles drift in straight lines determined solely by their initial velocity and drag. As gravity increases, trajectories curve into parabolic arcs â€” the classic shape of a projectile under constant acceleration. The gravity direction toggle determines whether particles are pulled downward (simulating falling embers) or upward (simulating rising sparks and hot gas). At maximum gravity, particles accelerate rapidly and exit the frame within a few frames of emission, producing short, sharp streaks rather than graceful arcs.
...
Global brightness multiplier applied to the final particle brightness before color mapping and compositing. At maximum, particles render at full intensity â€” the brightest cores saturate to white in Hot mode. At zero, all particle contribution is suppressed and only the input video passes through. Intermediate values scale the overall luminosity of the particle field, useful for matching the intensity of the ember overlay to the brightness range of the source video. This control acts as the master intensity for the entire particle system, distinct from the per-particle lifetime-based fade.
...
*Classic Fountain â€” simulated result across source images.*
...
4. **Flip gravity direction**: Toggle Gravity Dir to Up. The fountain now accelerates upward â€” particles launch and continue to climb, exiting the top of frame.
...
*Slow Ember Drift â€” simulated result across source images.*
...
5. **Observe color fade**: Watch individual particles transition from white through orange to deep red as their lifetime depletes â€” the incandescence curve in action.
...
*Video-Reactive Sparks â€” simulated result across source images.*
...
2. **Feed a high-contrast source**: Use footage with distinct bright highlights â€” candle flames, spotlights, bright text on dark backgrounds.
...
- **Size 1 for precision, Size 8 for atmosphere**: Single-pixel particles reveal the pure physics â€” every trajectory is a clean arc. Large particles create a soft, glowing haze where individual trajectories blur into a luminous field.
...
- **Hot palette at low brightness reveals the red end**: Pull the Brightness fader to 50â€“70% to expose the full orange-to-red gradient of the incandescence curve. At full brightness the white core dominates.
...
| **Euler integration** | The simplest numerical method for solving differential equations: new_position = position + velocity Ã— dt. Used here with dt = 1 frame. |
| **Lerp** | Linear interpolation between two values: result = a + (b âˆ’ a) Ã— t. Used in the additive compositing stage to blend particle color with the source video. |
