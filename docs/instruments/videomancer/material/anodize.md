...
description: "Anodize simulates the appearance of anodized aluminum — the electrochemical surface treatment that gives metal products their vivid, uniform colors whil..."
---
...
Anodize simulates the appearance of anodized aluminum — the electrochemical surface treatment that gives metal products their vivid, uniform colors while maintaining a distinctive metallic reflective quality. The program applies a saturated color tint across the entire image while preserving bright specular highlights, which remain white just as they do on real anodized metal surfaces. A metallic sheen effect couples brightness variations to chroma intensity, creating the characteristic luminance-dependent color shift seen on anodized surfaces. Optional grain adds subtle surface texture.

The hue is selected from four color quadrants in UV space — Red-Orange, Blue-Purple, Green-Teal, and Gold-Yellow — matching the most common anodized aluminum colors found in consumer electronics, architectural fixtures, and sporting equipment. A smooth mode interpolates between quadrants for continuous hue selection. The highlight threshold controls at what brightness level the tint begins to desaturate toward white, simulating specular reflections on the metal surface. Glossy mode pushes highlights further toward full desaturation, simulating a mirror-finish anodize.

At moderate settings, the program produces a convincing metallic tinted surface with natural highlight handling. At extreme settings — maximum saturation with low highlight threshold and full sheen — the image becomes a vivid metallic color study.
...
**Anodizing** is an electrochemical process that converts the surface of aluminum into a durable, corrosion-resistant aluminum oxide layer. This oxide layer is porous and can absorb dyes, allowing the metal to be colored in a wide range of hues. Unlike paint, the color is integrated into the surface itself — it cannot peel or chip because it IS the surface.

The most recognizable anodized products include Apple's MacBook Pro and iPhone housings (space gray, midnight, starlight), Leica camera bodies (black chrome), high-end bicycle frames, and architectural window frames. The distinctive quality of anodized color is its uniformity combined with the metallic surface underneath — the color is everywhere the same, but surface curvature and lighting create subtle brightness variations that the eye reads as "metal."

### What Are Specular Highlights on Anodized Metal?

The oxide layer on anodized aluminum is partially transparent. Light that penetrates the layer is absorbed and re-emitted at the dye color, creating the uniform tint. However, some light reflects directly off the outer surface without penetrating — these direct reflections are **specular highlights**, and they retain the color of the light source (typically white) rather than the dye color.

This is why anodized metal looks colored in general illumination but shows white highlights where light sources reflect directly. The program simulates this by detecting bright areas above a threshold and selectively reducing their color saturation toward neutral (512, 512 in UV = gray/white).

### What Is Metallic Sheen?

Real metal surfaces have a unique optical property: their reflected color varies with angle and curvature. This is because the reflected light is a mix of specular reflection (white) and diffuse reflection (colored). As the surface curves, the ratio changes, creating subtle color shifts that track the surface geometry. In video terms, this manifests as brightness-dependent color saturation — brighter areas are slightly more colored (or less colored, depending on the finish). The Sheen control simulates this by coupling luma deviations from midpoint to chroma offsets.
...
The hue tint target is computed from the Hue pot. In quadrant mode, the top 2 bits select one of four color regions in UV space. In smooth mode, bits 7 and 6 of the hue fraction independently control the U and V polarity, creating a continuous four-zone sweep through the color space. The Saturate pot controls the offset magnitude from the chroma midpoint (512).
...
Selects the anodize color hue. In quadrant mode (Color toggle), the pot selects between four distinct hue zones: Red-Orange (0-25%), Blue-Purple (25-50%), Green-Teal (50-75%), and Gold-Yellow (75-100%). In smooth mode, the pot sweeps continuously through the color space, with four distinct color regions blending at the boundaries. The hue determines the base color of the anodized surface.
...
Controls the saturation (color intensity) of the anodized tint. At 0%, the U and V channels stay at the neutral midpoint (512) — no color tint is applied. As saturation increases, the U and V values push further from midpoint toward the target hue. At maximum, the color is fully saturated — vivid and intense. Moderate values (40-60%) are most realistic for actual anodized aluminum.
...
Sets the luminance threshold above which specular highlight desaturation begins. Pixels brighter than this threshold have their color pulled toward neutral (gray/white). At low threshold, even moderate brightness triggers highlight whitening — the surface appears to have very bright, reflective highlights. At high threshold, only the very brightest pixels lose color — the anodize tint persists through most of the brightness range.
...
Controls the uniformity of the anodize application. This parameter modifies how evenly the tint is distributed across the surface. Higher values produce more uniform coloring, while lower values allow the underlying luminance to show through the tint more strongly.
...
Toggle 7 maps to the VHD's hue_mode bit — the TOML labels "Red/Blue/Green/Gold" correspond to the four quadrant mode colors, while the VHD actually uses bit 0 for quadrant vs smooth mode selection. This means the TOML labels guide the user to different Hue pot ranges rather than directly switching modes. Toggle 8 maps to glossy mode. Toggle 9 enables grain. Toggle 10 enables tint inversion.
...
These exercises progress from basic color tinting through highlight preservation to metallic surface simulation, demonstrating how to create convincing anodized aluminum appearances.
...
**Objective**: Understand how the hue quadrant and saturation controls create the basic anodized color tint.
...
**Key concepts**: Hue quadrant selection, saturation control, uniform color tinting in UV space
...
3. **Add sheen**: Set Sheen to ~60%. Notice how the color subtly shifts with brightness — darker areas lean one way, brighter areas another.
4. **Maximum sheen**: Push Sheen to ~90%. The metallic color variation is very pronounced.
5. **Add grain**: Toggle Hi Light to Sharp (enables grain via VHD mapping). Set Grain to ~30%.
6. **Observe texture**: A fine crystalline texture appears on the surface — especially visible on flat-color areas.
...
- **Moderate saturation is most realistic**: Real anodized aluminum has vivid but not neon-level color. Saturate at 40-60% looks most authentic.
...
- **Sheen at ~50% is the sweet spot**: Enough metallic variation to read as metal, not so much that it overwhelms the color.
...
- **Red and Gold are the most common**: These are the colors most people associate with anodized aluminum from consumer products.
...
| Term | Definition |
|------|------------|
| **Anodizing** | An electrochemical process that converts aluminum's surface into a durable, porous oxide layer capable of absorbing dye color; the program simulates this uniform tinted-metal appearance. |
| **Chroma** | The color-difference components (U and V) of a YUV signal, encoding hue and saturation independently of brightness. |
| **Desaturation** | Reducing a pixel's color intensity by pulling its U and V values toward the neutral midpoint (512), making it appear more gray or white. |
| **FPGA** | Field-Programmable Gate Array; the reconfigurable hardware chip that implements Videomancer's real-time video processing. |
| **Luminance (Luma)** | The brightness component (Y channel) of a YUV video signal. |
| **Metallic Sheen** | A brightness-dependent color shift that emulates how real metal surfaces vary in color with viewing angle; implemented by coupling luma deviations to chroma offsets. |
| **Quadrant Mode** | A hue-selection scheme where the 10-bit pot range is divided into four zones, each producing a distinct color family (Red-Orange, Blue-Purple, Green-Teal, Gold-Yellow). |
| **Specular Highlight** | A direct surface reflection of a light source that retains the light's color (white) rather than the surface tint, because the light reflects before penetrating the dyed oxide layer. |
| **UV Space** | The two-dimensional chrominance plane of the YUV color model, where angle from centre encodes hue and distance from centre encodes saturation. |
