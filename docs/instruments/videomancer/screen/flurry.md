...
import flurry_exercise1_result from '/img/instruments/videomancer/flurry/flurry_exercise1_result.png';
import flurry_exercise2_result from '/img/instruments/videomancer/flurry/flurry_exercise2_result.png';
import flurry_exercise3_result from '/img/instruments/videomancer/flurry/flurry_exercise3_result.png';
...
*Flurry output evolving over multiple frames — synthesis programs generate imagery without requiring a video input source.*
...
│   ├─ orbit_radius = registers_in(0)   [10-bit]
│   ├─ speed        = registers_in(1)   [10-bit]
│   ├─ size         = registers_in(2)   [10-bit]
│   ├─ color        = registers_in(3)   [10-bit]
│   ├─ brightness   = registers_in(4)   [10-bit]
│   ├─ decay        = registers_in(5)   [10-bit, unused in VHDL]
│   ├─ mix_amount   = registers_in(7)   [10-bit]
│   └─ toggles from registers_in(6):
│       ├─ bit 0: color_cycle (0=off, 1=on)
│       ├─ bit 1: trails      (0=off, 1=on) [unused in VHDL]
│       ├─ bit 2: multi_orbit (0=off, 1=on) [unused in VHDL]
│       ├─ bit 3: blur        (0=off, 1=on) [unused in VHDL]
│       └─ bit 4: bypass
│
├── Quarter-Wave Sine LUT (32 entries, 8-bit unsigned) ────────
│   └─ Values: 0, 25, 50, 74, 98, 120, 142, 162,
│              180, 197, 212, 225, 236, 244, 250, 254,
│              255, 254, 250, 244, 236, 225, 212, 197,
│              180, 162, 142, 120, 98, 74, 50, 25
│
├── Phase Accumulator (16-bit, per-vsync update) ──────────────
│   └─ s_phase_acc += speed(9 downto 2)  [upper 8 bits of Speed]
│
├── Orbit Position Computation (per vsync) ────────────────────
│   ├─ phase_idx = s_phase_acc(15 downto 11)  [upper 5 bits]
│   ├─ sin_val = sine_lut(phase_idx)
│   ├─ cos_val = sine_lut(phase_idx + 8)  [quarter-cycle offset]
│   ├─ orbit_x = 640 + (sin_val × orbit_radius(9:2)) >> 8
│   └─ orbit_y = 360 + (cos_val × orbit_radius(9:2)) >> 8
│
├── Per-Pixel Manhattan Distance ──────────────────────────────
│   ├─ dx = |x_counter − orbit_x|
│   ├─ dy = |y_counter − orbit_y|
│   └─ dist = dx + dy
│
├── Glow Generation ───────────────────────────────────────────
│   └─ if dist < size:
│       └─ glow = brightness − dist(9 downto 0)
│   else:
│       └─ glow = 0
│
├── Color Assignment ──────────────────────────────────────────
│   ├─ proc_y = glow
│   ├─ Color Cycle ON:
│   │   ├─ proc_u = frame_counter(9 downto 2)  [8-bit, expanded to 10]
│   │   └─ proc_v = frame_counter(7 downto 0)  [8-bit, expanded to 10]
│   └─ Color Cycle OFF:
│       ├─ proc_u = color
│       └─ proc_v = 1023 − color
│
...
The architecture is driven by two distinct temporal domains. The orbit engine runs exclusively during the vertical blanking interval: each vsync falling edge advances the 16-bit phase accumulator by the upper 8 bits of the Speed register, looks up sine and cosine values from the 32-entry LUT, multiplies each by the scaled orbit radius, and adds the results to the screen center coordinates (640, 360). The cosine component uses a phase index offset of +8 entries within the same sine table — equivalent to a 90° phase shift — which produces the elliptical orbit geometry. The rendering pipeline then operates during active video, computing the Manhattan distance from each output pixel to the single particle position and generating a glow contribution based on the Size and Brightness parameters.

The color cycling mechanism, when enabled, derives chrominance directly from the free-running 16-bit frame counter rather than from the Color knob. The U channel receives bits 9:2 of the counter (a slowly changing 8-bit value expanded to the 10-bit register width) while V receives bits 7:0 — a faster-cycling 8-bit value. Because these two bit fields overlap partially (bits 7:2 are shared), U and V evolve at different rates, producing a continuous, asymmetric color drift that cycles through a complex path in the UV chrominance plane. When color cycling is disabled, U is set directly from the Color parameter while V is computed as 1023 − Color, producing complementary chrominance that sweeps from one end of the UV axis to the other as the knob is turned.
...
*Videomancer's front panel with Flurry active. Knobs 1–6 (top two rows of left cluster), Toggle switches 7–11 (bottom row of left cluster), Fader 12 (right side).*
...
Controls the radial extent of the particle's elliptical orbit around the screen center at (640, 360). The upper 8 bits of the 10-bit register value are multiplied by the sine and cosine LUT outputs and right-shifted by 8 to produce pixel offsets from center. At minimum, the orbit collapses to a stationary point at screen center — the multiplication product is near zero regardless of the trigonometric values. At maximum, the particle sweeps wide arcs that can approach the screen edges, creating dramatic spatial coverage. Intermediate values produce compact to moderate ellipses whose aspect ratio is fixed by the sine/cosine offset relationship in the LUT.
...
Determines the spatial extent of the glow diamond rendered around the particle center. The Size register value serves as the distance threshold: pixels whose Manhattan distance from the particle center is less than this value receive a glow contribution, while pixels beyond the threshold render as black. At minimum, the glow collapses to a tiny point. At maximum, the diamond extends 1023 pixels in Manhattan distance from the particle center, covering a substantial fraction of the screen. Size interacts with Brightness to determine the visible extent — the effective visible radius is the minimum of Size and Brightness, since the glow formula (brightness − distance) reaches zero when distance equals brightness even if Size allows larger distances.
...
When Color Cycle is Off, this knob sets the fixed chrominance of the particle. The U channel receives the Color value directly while V receives 1023 − Color, creating a complementary relationship. At the center position (512), U and V are both near midpoint, producing a near-neutral white glow. Turning the knob toward minimum shifts the color toward one end of the UV axis (low U, high V — warm tones), while turning toward maximum shifts it to the opposite end (high U, low V — cool tones). When Color Cycle is On, this parameter has no effect — chrominance is derived from the frame counter instead.
...
| Switch | Off | On |
|--------|-----|-----|
| **7 — Color Cycle** | Off | On |
| **8 — Trails** | Off | On |
| **9 — Multi Orbit** | Off | On |
| **10 — Blur** | Off | On |
| **11 — Bypass** | Off | On |

Toggles 7–11 configure five binary aspects of the particle system, though only two toggles have behavioral effects in the current VHDL. Color Cycle (7) switches between fixed knob-derived chrominance and frame-counter-modulated color drift. Bypass (11) routes delayed input directly to the output. The remaining three toggles — Trails (8), Multi Orbit (9), and Blur (10) — are wired to VHDL signals but have no effect on the rendering pipeline. They serve as reserved interface points for future extensions: Trails would enable temporal persistence, Multi Orbit would render additional particles on offset paths, and Blur would soften the diamond glow edges.
...
5. **Observe U vs V cycling rates**: U derives from frame_counter bits 9:2 (slowly changing) while V derives from bits 7:0 (faster changing). The particle's hue should drift through a complex path — sometimes warm, sometimes cool, sometimes neutral.
...
5. **Observe glow extent**: At maximum size and brightness, the diamond should cover a very large area of the screen — brightness drops linearly from 1023 at center to 0 at a Manhattan distance of 1023.
...
8. **Reduce brightness gradually**: With size still moderate, sweep Brightness down and observe the glow shrinking even within the size threshold — the linear falloff reaches zero sooner at lower brightness.
...
| **Elliptical orbit** | A closed curve traced by a point moving under parametric sine/cosine control with a phase offset between the two components. The 8-entry offset in Flurry's LUT produces the 90° separation between horizontal and vertical motion that creates the ellipse. |
| **Manhattan distance** | A distance metric computed as $|dx| + |dy|$ — the sum of absolute differences along each axis. Named after Manhattan's grid street layout. Produces diamond-shaped iso-distance contours rather than circles. |
