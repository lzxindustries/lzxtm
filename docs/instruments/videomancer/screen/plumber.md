...
description: "Before Windows had wallpaper engines, it had screensavers."
---
...
### Growth, Color Cycling, and Canvas Clearing

The growth cursor carries a 3-bit color counter that increments each time it teleports, cycling through 8 palette entries. This means each contiguous run of pipe segments shares a color, and color changes mark the points where the cursor jumped to a new location. The canvas is cleared when the cell count reaches 75% of the total grid (60 × 34 × 0.75 = 1530 cells), triggering a sequential memory wipe that zeroes one cell per clock. After clearing, the cursor resumes from a random position with the accumulated color counter.
...
├─ reg(6)(1) → Video Fill    (solid color / video texture)
...
│   │       └─ Normal → palette[color_idx] Y/U/V
│   ├─ Grid line pixel:
...
This parameter is mapped to register 5 (`s_hue_pot`) but is **not referenced** by the rendering pipeline. The pipe color for each segment is determined by an internal 8-entry palette indexed by the growth cursor's 3-bit color counter, which increments on each teleport. The Hue control currently has no visible effect on the output. It is retained in the register map for potential future use or custom VHDL modifications.
...
**Key concepts**: Growth Rate controls ticks per second, Straight biases the random walk toward continuation, each seed produces a deterministic network, grid lines reveal the cell structure, color changes mark teleport events
...
1. **Feed video**: Connect a video source with colorful, high-contrast content.
...
**Key concepts**: Video Fill replaces pipe color with input video, background dim separates pipe regions from surroundings, grid lines add framing, the pipe footprint acts as a stencil masking the video, high Straight creates corridor-like compositions
...
- **Hue pot is currently unused**: The Pipe Hue knob has no effect on the output. Pipe color cycles automatically through 8 colors as the cursor teleports.
...
| **Teleport** | The growth cursor's fallback behaviour when all four cardinal neighbours are occupied or out of bounds. The cursor jumps to a randomly selected empty cell and continues in a new direction with an incremented color. |
